	...
	try
	{
		...
		while (true) {
			for (int j = 0; j < n; j++) {
				...
				{
					//je�linei idziemy w przeciwnym kierunku
					X_temp.x = XB.x - (s(i) * d[i]);
					X_temp.fit_fun(ff);
					if (X_temp.y < XB.y)
					{
			...
			//przerywamy je�li osi�gni�to wystarczajaca dok�adno��
			if (s(i) < epsilon)
			{
				break;
			}
			...
			Xopt.y = XB.y;
			Xopt.flag = 1;
			std::cout << "Aktualne warto�ci Rosen: x = " << XB.x << ", y = " << XB.y << ", s(i) = " << s(i) << std::endl;
			return Xopt;
